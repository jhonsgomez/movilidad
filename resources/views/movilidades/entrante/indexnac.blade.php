...
@section('title', 'Movilidades Nacionales Entrantes')
...
<form action="" class="border border-2 rounded-3 shadow-lg mt-5 mb-5" style="width: 95%;">
    ...
    <div class="row mt-4 p-3 shadow-lg rounded-3 titles">
        <div class="offset-1 col-10">
            <h4 class="text-center ">Movilidades Nacionales Entrantes</h4>
        </div>
    </div>
    <div class="row mt-4">
        <div class="offset-1 col-10">
                        ...
                        <tbody>
                            @foreach ($movnaent as $item)
                                <tr><td class="text-center"> {{ $item->id }} </td>
                                    ...
                                    @if (auth()->user()->rol_id == 2 || auth()->user()->rol_id == 6 || $item->user_id == auth()->user()->id)
                                    <td>
                                        <div class="row">
                                            <div class="col">
                                                <a class="btn btn-primary  w-100" href="{{ route('movilidadNacEnt.edit', $item->id) }}">Editar</a>
                                            </div>
                                        </div>
                                        <form action="{{route('movilidadNacEnt.destroy', $item->id)}}" method="POST" class="form-delete">
                                            @csrf
                                            <div class="row mt-1">
                                                <div class="col">
                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form action="{{ route('movNacEnt.export') }}">
                            <div class="form-group mb-2">
                                <label for="desde">Desde:</label>
                                <input type="date" class="form-control" name="mne_initialDate" id="mie_initialDate">
                            </div>
                            <div class="form-group mb-2">
                                <label for="desde">Hasta:</label>
                                <input type="date" class="form-control" name="mne_finalDate" id="mie_initialDate" >
                            </div>
