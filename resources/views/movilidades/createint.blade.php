...
@section('content')
    <form method="POST" class="form-conv-nac border border-2 rounded-3 shadow-lg mt-5 mb-5" action="{{ route('movilidades_int.store') }}" enctype="multipart/form-data">
        @csrf
        <div class="row mt-3 p-3 shadow-lg rounded-3 titles">
            <div class="offset-1 col-10">
                <h4 class="text-center" id="ori">Registro Movilidad Internacional</h4>
            </div>
        </div>
        <div class="row mt-4">
            ...
            <div class="offset-1 col-10">
                ...
                <select class="form-select border-dark" name="inst_ent_ints" id="inst_ent_ints">
                    ...
                    @foreach ($instituciones as $institucion)
                        <option value="{{ $institucion->id }}" {{ old('inst_ent_ints') == $institucion->id ? 'selected': '' }}> {{ strtoupper($institucion->nombre) }}</option>
                    @endforeach
                </select>
                @error('inst_ent_ints')
                    <span class="text-danger">*{{ $message }}</span>    
