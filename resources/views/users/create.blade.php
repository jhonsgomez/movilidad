...
@section('content')
    <form method="POST" action="{{ route('users.store') }}" style="width: 30% ;" class="border border-2 rounded-3 shadow-lg">
        @csrf
        ...
        <div class="row mt-4">
            <div class="offset-1 col-10">
                <input type="text" name="first_name" id="first_name" class="form-control" placeholder="* Primer nombre..." required value="{{ old('first_name') }}">
            </div>
        </div>
        <div class="row mt-4">
            <div class="offset-1 col-10">
        ...
        <div class="row mt-4">
            <div class="offset-1 col-10">
                <input type="text" name="last_name" id="last_name" class="form-control" placeholder="* Apellido(s)..." required value="{{ old('last_name') }}">
            </div>
        </div>
        <div class="row mt-4">
            <div class="offset-1 col-10">
                <select name="rol" id="rol" class="form-select" required>
                    ...
                    @foreach ($roles as $item)
                        <option value="2" {{ old('rol') == $item->id ? 'selected': '' }}>{{ $item->rol_name }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="row mt-4">
            <div class="offset-1 col-10">
                <div class="input-group">
                    <input type="password" name="password" id="password" class="form-control" placeholder="* Contraseña..." required pattern="^[A-Z](?=.*\d)(?=.*?[#?!@$%^&*-])(?=.*[A-Z])(?=.*[a-z])\S{8,16}$">
